...
В настройках можно выбрать стиль разметки (MBR, GPT), выделить часть устройства в отдельный раздел, удалить **Ventoy**. Мы же нажимаем кнопку *Установить* и соглашаясь на <span style="color: red;">форматирование устройства</span>, устанавливаем программу.
...
OpenMediaVault | [ссылка](https://www.openmediavault.org/?page_id=77) | Тут выбираем версию и скачиваем файл через *SourceForge*
Ubuntu Server 22.04 | []() | 
Debian | []() |
Xubuntu 22.04 | []() |
...
№ |ID в Proxmox| *lsblk* | Предназначение
--:|:--|:--|:--
1. | ***local-lvm*** | `/dev/pve/data` | rootdir, images
2. | ***local*** | `/dev/pve/root` | iso,vztmpl,backup
...
+ При установке Proxmox в момент выбора носителя, нажимаем *Options* и задаём максимум на *root* - 15G, на *swap* - 0, минимальное свободное пространство - 37G.
+ После перезагрузки сервера заходим в терминал и корректируем файл `/etc/pve/storage.cfg`.
...
Теперь можно обновить пакеты, но сначала нужно подключить бесплатные репозитории. Для этого открываем `/etc/apt/sources.list.d/pve-enterprise.list` и оставляем в таком виде:
...
Будем создавать машину через веб интерфейс. Самое главное - это не указывыать носитель во вкладке `ОС` (*нет носителя*), поставить галку qemu-agent в системе и удалить предложенный диск.
...
***/etc/netplan/<settings_file>.yaml***
```yaml
network:
    version: 2
    ethernets:
