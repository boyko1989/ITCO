...
Начнём с предшественника данного сервиса - **INT 13H**. Этот сервис предоставляет прямой доступ а адаптерам дискеты и твердого диска. Это один из вызовов обработчика прерываний BIOS.

Чтобы понять, что это такое нужно прийти к пониманию того, как устроена адресация памяти. Экскурс в историю делать не будем (о памяти [здесь](../../../administration/00-hard/arch/memory/README.md)), поэтому
